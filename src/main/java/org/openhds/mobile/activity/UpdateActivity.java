 ...
 */
public class UpdateActivity extends Activity implements ValueFragment.ValueListener, LoaderCallbacks<Cursor>, EventFragment.Listener,
		SelectionFragment.Listener {
	...
	class CheckLocationFormStatus extends AsyncTask<Void, Void, Boolean> {
		...
		protected Boolean doInBackground(Void... arg0) {
			Cursor cursor = resolver.query(contentUri, new String[] { InstanceProviderAPI.InstanceColumns.STATUS,
					InstanceProviderAPI.InstanceColumns.INSTANCE_FILE_PATH }, InstanceProviderAPI.InstanceColumns.STATUS + "=?",
	...
	private void startFilterActivity(int requestCode) {
		...
		case 9:
			...
			i.putExtra("hierarchy7", locationVisit.getHierarchy7());
			i.putExtra("hierarchy8", locationVisit.getHierarchy8());
			i.putExtra("hierarchy9", locationVisit.getHierarchy9());
			break;
		}
	...
	private void createUnfinishedFormDialog() {
		...
		if (xformUnfinishedDialog == null) {
			...
			alertDialogBuilder.setMessage("Form started but not saved. " + "This form instance will be deleted. What do you want to do?");
	...
	public Loader<Cursor> onCreateLoader(int id, Bundle args) {
		...
		case SOCIAL_GROUP_FOR_EXT_INMIGRATION:
			uri = OpenHDS.SocialGroups.CONTENT_LOCATION_ID_URI_BASE.buildUpon().appendPath(locationVisit.getLocation().getExtId()).build();
			break;
	...
	public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
		...
		SimpleCursorAdapter adapter = new SimpleCursorAdapter(this, android.R.layout.simple_list_item_2, cursor, new String[] {
				OpenHDS.SocialGroups.COLUMN_SOCIALGROUP_GROUPNAME, OpenHDS.SocialGroups.COLUMN_SOCIALGROUP_EXTID }, new int[] {
	...
	private int getNumHierarchies() {
		if (sp == null || numHierarchies == null) {
			sp = PreferenceManager.getDefaultSharedPreferences(this.getBaseContext());
			numHierarchies = sp.getString(LocationHierarchyActivity.NUM_HIERARCHIES, "4");
			numHierarchiesInt = Integer.parseInt(numHierarchies);
		}
		return numHierarchiesInt;
	}
