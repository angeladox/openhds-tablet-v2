 ...
 * entity can be defined as: 1-9 Location Hierarchies, Round, Location and
 * Individual
 */
public class ValueFragment extends ListFragment implements
	...
	// load
	private static final String[] HIERARCHY_COLUMNS = new String[] {
			OpenHDS.HierarchyItems.COLUMN_HIERARCHY_NAME,
			OpenHDS.HierarchyItems.COLUMN_HIERARCHY_EXTID };
	private static final String[] ROUNDS_COLUMNS = new String[] {
	...
	private static final String[] INDIVIDUAL_COLUMNS = new String[] {
	...
	private enum Displayed {
		HIERARCHY_1, HIERARCHY_2, HIERARCHY_3, HIERARCHY_4, HIERARCHY_5, HIERARCHY_6, HIERARCHY_7, HIERARCHY_8, HIERARCHY_9, ROUND, LOCATION, INDIVIDUAL;
	}
	...
	public void onAttach(Activity activity) {
		...
		adapter = new SimpleCursorAdapter(getActivity(),
				android.R.layout.simple_list_item_2, null, HIERARCHY_COLUMNS,
				VIEW_BINDINGS, 0);
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		...
		case HIERARCHY_4:
			...
			break;
		case HIERARCHY_5:
			LocationHierarchy locHierarchy5 = Converter
					.convertToHierarchy(cursor);
			listener.onHierarchy5Selected(locHierarchy5);
			break;
		case HIERARCHY_6:
			LocationHierarchy locHierarchy6 = Converter
					.convertToHierarchy(cursor);
			listener.onHierarchy6Selected(locHierarchy6);
			break;
		case HIERARCHY_7:
			LocationHierarchy locHierarchy7 = Converter
					.convertToHierarchy(cursor);
			listener.onHierarchy7Selected(locHierarchy7);
			break;
		case HIERARCHY_8:
			LocationHierarchy locHierarchy8 = Converter
					.convertToHierarchy(cursor);
			listener.onHierarchy8Selected(locHierarchy8);
			break;
		case HIERARCHY_9:
			LocationHierarchy locHierarchy9 = Converter
					.convertToHierarchy(cursor);
	...
	public Loader<Cursor> onCreateLoader(int arg0, Bundle arg1) {
		...
		String START_HIERARCHY_LEVEL_NAME = sp.getString(
				LocationHierarchyActivity.HIERARCHY_1, getString(R.id.hierarchy1));
		switch (arg0) {
		case HIERARCHY_LOADER:
			adapter.changeCursorAndColumns(null, HIERARCHY_COLUMNS,
					VIEW_BINDINGS);
			return new CursorLoader(getActivity(),
					OpenHDS.HierarchyItems.CONTENT_ID_URI_BASE, null,
		...
		case ROUND_LOADER:
			...
			return new CursorLoader(getActivity(),
					OpenHDS.Rounds.CONTENT_ID_URI_BASE, null, null, null, null);
		case LOCATION_LOADER:
			adapter.changeCursorAndColumns(null, LOCATION_COLUMNS,
					VIEW_BINDINGS);
		...
		case INDIVIDUAL_LOADER:
			adapter.changeCursorAndColumns(null, INDIVIDUAL_COLUMNS,
					VIEW_BINDINGS);
			return new CursorLoader(getActivity(),
					OpenHDS.Individuals.CONTENT_ID_URI_BASE, null,
		...
		case INDIVIDUAL_FILTER_LOADER:
			...
			return new CursorLoader(getActivity(),
					OpenHDS.Individuals.CONTENT_ID_URI_BASE, null, filter,
					args, OpenHDS.Individuals.COLUMN_INDIVIDUAL_EXTID + " ASC");
		}
	...
	private Loader<Cursor> buildRegionCursorLoader(Bundle arg1) {
		if (TextUtils.isEmpty(arg1.getString("parentExtId"))) {
			return buildCursorLoader(
					OpenHDS.HierarchyItems.CONTENT_ID_URI_BASE, null, null);
		} else {
			return buildCursorLoader(
	...
	private Loader<Cursor> buildCursorLoader(Uri uri, String where,
			String[] args) {
	...
	private String buildFilter(Bundle arg1) {
		...
		if (!TextUtils.isEmpty(arg1.getString("firstName"))) {
			if (builder.length() > 0)
				builder.append(" AND ");
			builder.append(OpenHDS.Individuals.COLUMN_INDIVIDUAL_FIRSTNAME
					+ " LIKE ?");
		}
		if (!TextUtils.isEmpty(arg1.getString("lastName"))) {
			if (builder.length() > 0)
				builder.append(" AND ");
			builder.append(OpenHDS.Individuals.COLUMN_INDIVIDUAL_LASTNAME
					+ " LIKE ?");
		}
		if (!TextUtils.isEmpty(arg1.getString("gender"))) {
			if (builder.length() > 0)
	 ...
	 */
	public void loadFilteredIndividuals(String location, String firstName,
		...
		getLoaderManager()
				.restartLoader(INDIVIDUAL_FILTER_LOADER, bundle, this);
	}
